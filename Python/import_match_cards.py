...
#python3 .\Python\import_match_cards.py
...
try:
    ...
    sqlCommandRead = 'SELECT id, card FROM Match WHERE card <> "" ;'
    ...
    for row in output:
        ...
        #Carrega os dados XML da coluna card
        ...
        for level in levels:
          ...
          if level.find('player1') != None and level.find('team') != None and level.find('card_type') != None :

            #Insere os valores na tupla
            myTuple =  (vMatchId, level.find('team').text,  level.find('player1').text,  level.find('card_type').text)
    ...
    sqlCommandInsert = 'INSERT INTO Match_Cards ( match_id, team_api_id, player_api_id, card_type) VALUES (?,?,?,?);'
